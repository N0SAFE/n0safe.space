...
export default class Client implements AdvancedSocketMethods {
    ...
    emitOnClusterConnection(evt: string, callback: (() => any[]) | (() => Promise<any[]>) | any, ...args: any[]) {
    ...
    emitOnServerConnection(evt: string, callback: (() => any[]) | (() => Promise<any[]>) | any, ...args: any[]) {
    ...
    connect(newInfo: IInfo) {
        ...
        this.socket = io(`${this.info.protocol}://${this.info.host}:${this.info.port}${this.info.path}`, {
            autoConnect: true,
            reconnection: true
        });
        console.log(`connecting to ${this.info.protocol}://${this.info.host}:${this.info.port}${this.info.path}`);
    ...
    removeAllListeners(list: (string | (() => void))[] = [], listType: "black" | "white" = "black") {
        const socket = this.socket as any;
        const callbacksList: [string, (() => void)[]][] = Object.entries(socket._callbacks);
        ...
        callbacksList.forEach(([event, callbacks]) => {
            ...
            [...callbacks].forEach((callback) => {
                if (list.includes(event) || list.includes(callback)) {
                    if (listType === "white" && ![this.connectErrorFn, this.connectFn, this.disconnectFn].includes(callback)) {
                        socket.off(event, callback);
                    }
                    return;
                }
                if (listType === "black" && ![this.connectErrorFn, this.connectFn, this.disconnectFn].includes(callback)) {
                    socket.off(event, callback);
                }
            });
        });
