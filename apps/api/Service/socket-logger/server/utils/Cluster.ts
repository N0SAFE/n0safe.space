...
import type { IsHttpServer } from '../../utils/types'
...
export default class Cluster<GlobalStore, ServerStore> extends Server {
  ...
  public serverExists (port: number, path?: string) {
    if (path) {
      return this.servers.some((server) => server.port === port && server.path() === path)
    }
  ...
  public pathIsAllowed (path: string) {
    return this.serversInfo.some((serverInfo) =>
      serverInfo.path instanceof RegExp ? serverInfo.path.test(path) : serverInfo.path === path
    )
  }

  public redirect (server: Server, info: { port: number; path?: string }, payload?: any) {
    server.emit('redirect', {
      type: 'redirected',
      data: 'redirected to port ' + info.port + (info.path && ' and path ' + info.path),
      port: info.port,
      path: info.path || '',
      payload: payload,
    })
